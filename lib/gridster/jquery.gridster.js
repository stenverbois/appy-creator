...
(function (root, factory) {
	...
	function Gridster (el, options) {
		...
		this.min_row_count = this.options.min_rows;
		this.prev_row_count = this.min_row_count;
	...
	fn.resize_widget = function ($widget, size_x, size_y, callback) {
		...
		if (!this.is_valid_col(wgd.col, size_x)) {
			this.add_faux_cols(Math.max(this.calculate_highest_col(wgd.col, size_x) - this.cols, 0));
		}
 ...
 * @param {Function} callback The function to execute on each column
	...
	fn.addVueComp = function(vueObj) {
		// APPY
		// this.$widgets += this.$el.children(this.options.widget_selector).addClass('gs-w');


		var $w = this.$el.children('li').last();
